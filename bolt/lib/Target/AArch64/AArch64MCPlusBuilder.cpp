...
static void createPushRegisters(MCInst &Inst, MCPhysReg Reg1, MCPhysReg Reg2) {
  // Create stp Xt1, Xt2, [sp, #-16]! instruction
  // According to AArch64InstrFormats.td, StorePairPreIdx has:
  // (outs GPR64sp:$wback) - output operand
  // (ins regtype:$Rt, regtype:$Rt2, GPR64sp:$Rn, indextype:$offset) - input operands
  // So the correct order is: wback (output), Rt, Rt2, Rn, offset
  Inst.clear();
  unsigned NewOpcode = AArch64::STPXpre;
  Inst.setOpcode(NewOpcode);
  Inst.addOperand(MCOperand::createReg(AArch64::SP));  // Op0: wback (output, SP)
  Inst.addOperand(MCOperand::createReg(Reg1));          // Op1: Rt (first register, e.g., FP/X29)
  Inst.addOperand(MCOperand::createReg(Reg2));          // Op2: Rt2 (second register, e.g., LR/X30)
  Inst.addOperand(MCOperand::createReg(AArch64::SP));  // Op3: Rn (base register, SP)
  Inst.addOperand(MCOperand::createImm(-2));            // Op4: offset (-2 * 8 = -16 bytes, pre-indexed)
}

static void createPopRegisters(MCInst &Inst, MCPhysReg Reg1, MCPhysReg Reg2) {
  // Create ldp Xt1, Xt2, [sp], #16 instruction
  // According to AArch64InstrFormats.td, LoadPairPostIdx has:
  // (outs GPR64sp:$wback) - output operand
  // (ins regtype:$Rt, regtype:$Rt2, GPR64sp:$Rn, indextype:$offset) - input operands
  // So the correct order is: wback (output), Rt, Rt2, Rn, offset
  Inst.clear();
  ...
  Inst.addOperand(MCOperand::createReg(Reg2));          // Op2: Rt2 (second register, e.g., LR/X30)
  Inst.addOperand(MCOperand::createReg(AArch64::SP));  // Op3: Rn (base register, SP)
  Inst.addOperand(MCOperand::createImm(2));             // Op4: offset (2 * 8 = 16 bytes, post-indexed)
}
...
public:
  ...
  void createPushRegisters(MCInst &Inst, MCPhysReg Reg1, MCPhysReg Reg2) const override {
    // Create stp Xt1, Xt2, [sp, #-16]! instruction
    // According to AArch64InstrFormats.td, StorePairPreIdx has:
    // (outs GPR64sp:$wback) - output operand
    // (ins regtype:$Rt, regtype:$Rt2, GPR64sp:$Rn, indextype:$offset) - input operands
  ...
  void createPopRegisters(MCInst &Inst, MCPhysReg Reg1, MCPhysReg Reg2) const override {
    // Create ldp Xt1, Xt2, [sp], #16 instruction
    // According to AArch64InstrFormats.td, LoadPairPostIdx has:
    // (outs GPR64sp:$wback) - output operand
    // (ins regtype:$Rt, regtype:$Rt2, GPR64sp:$Rn, indextype:$offset) - input operands
  ...
  bool isTerminateBranch(const MCInst &Inst) const override {
    // AArch64 doesn't have a specific terminate branch instruction like x86
    // Return false for all instructions (conservative approach)
    return false;
  }
