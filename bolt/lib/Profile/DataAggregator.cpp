...
std::error_code DataAggregator::parseMMapEvents() {
  ...
  for (MMapInfo &MMapInfo : llvm::make_second_range(make_range(Range))) {
    ...
    if (!BinaryMMapInfo.insert(std::make_pair(MMapInfo.PID, MMapInfo)).second) {
      // For PIE binaries on aarch64, base addresses may differ between segments
      // This is a known issue, so we allow it and use the first base address
      if (MMapInfo.BaseAddress != BinaryMMapInfo[MMapInfo.PID].BaseAddress) {
        errs() << "PERF2BOLT-WARNING: Base address mismatch for PID " << MMapInfo.PID
               << " (expected 0x" << Twine::utohexstr(BinaryMMapInfo[MMapInfo.PID].BaseAddress)
               << ", got 0x" << Twine::utohexstr(MMapInfo.BaseAddress)
               << "). Using first base address.\n";
        // Continue with existing base address
      }
    }
