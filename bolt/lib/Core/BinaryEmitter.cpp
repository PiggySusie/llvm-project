...
void BinaryEmitter::emitFunctionBody(BinaryFunction &BF, FunctionFragment &FF,
  ...
  for (BinaryBasicBlock *const BB : FF) {
    ...
    for (auto I = BB->begin(), E = BB->end(); I != E; ++I) {
      ...
      // [调试] 对于注入函数，输出指令信息以便追踪 prologue
      if (BF.isInjected() && !EmitCodeOnly) {
        const unsigned Opcode = Instr.getOpcode();
        // STPXpre = 1696, LDPXpost = 1697
        if (Opcode == 1696 || Opcode == 1697) {
          BC.outs() << "BOLT-EMIT-DEBUG: Emitting prologue/epilogue instruction in injected function: "
                    << BF.getPrintName() << "\n";
          BC.outs() << "  Opcode: " << Opcode << " (1696=STPXpre, 1697=LDPXpost)\n";
          BC.outs() << "  Operands: " << Instr.getNumOperands() << "\n";
          BC.outs() << "  isPseudo: " << BC.MIB->isPseudo(Instr) << "\n";
          BC.outs() << "  isCFI: " << BC.MIB->isCFI(Instr) << "\n";
        }
      }
